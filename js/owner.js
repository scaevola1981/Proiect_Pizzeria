...
// ==========================================
// GESTIONARE TABLETE (Ospătari)
// ==========================================

window.openTableteModal = function () {
    const modal = document.getElementById('tablete-modal');
    if (modal) modal.classList.remove('hidden');
    window.loadActiveDevices();
};

window.closeTableteModal = function () {
    const modal = document.getElementById('tablete-modal');
...
window.loadActiveDevices = async function () {
    ...
    if (!container) return;
    container.innerHTML = '<p style="color: #fff; text-align: center;">Se încarcă lista de tablete...</p>';
    ...
    if (error) {
        console.error("Eroare la preluarea tabletelor:", error);
        container.innerHTML = '<p style="color: #e74c3c; text-align: center;">Eroare la încărcare.</p>';
        return;
    }

    if (!data || data.length === 0) {
        container.innerHTML = '<p style="color: #fff; text-align: center;">Nu există nicio tabletă înregistrată momentan.</p>';
        return;
    }
    ...
    data.forEach(dev => {
        ...
        const statusText = isPending ? 'Așteaptă Activarea' : 'Activă (Ospătar)';
        const name = dev.custom_name || dev.device_id;
        ...
        html += `
            <div style="background: rgba(255,255,255,0.1); border-radius: 10px; padding: 15px; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center;">
                <div style="display: flex; flex-direction: column;">
                    <strong style="color: #f5b041; font-size: 1.1rem; cursor: pointer;" onclick="window.renameDevice('${dev.id}', '${name}')" title="Click pentru a redenumi">${escapeHTML(name)} <i class="fas fa-edit" style="font-size: 0.8rem; margin-left: 5px;"></i></strong>
                    <span style="color: ${color}; font-size: 0.9rem; margin-top: 5px;"><i class="fas fa-circle" style="font-size: 0.6rem; vertical-align: middle;"></i> ${statusText}</span>
                    <span style="color: #95a5a6; font-size: 0.8rem;">ID Tehnic: ${escapeHTML(dev.device_id)}</span>
                </div>
                <div style="display: flex; gap: 8px; align-items: center;">
                    <button onclick="window.renameDevice('${dev.id}', '${escapeHTML(name).replace(/'/g, "\\'")}')" style="background: #3498db; color: white; border: none; padding: 8px 12px; border-radius: 8px; cursor: pointer; font-weight: bold; font-size: 0.9rem;" title="Schimbă Numele Tabletei">
                        <i class="fas fa-pen"></i> Redenumește
                    </button>
                    ${isPending 
                        ? `<button onclick="window.setDeviceStatus('${dev.id}', 'active')" style="background: #2ecc71; color: white; border: none; padding: 8px 15px; border-radius: 8px; cursor: pointer; font-weight: bold; font-size: 0.9rem;">Activează</button>`
                        : `<button onclick="window.setDeviceStatus('${dev.id}', 'pending')" style="background: #e74c3c; color: white; border: none; padding: 8px 15px; border-radius: 8px; cursor: pointer; font-weight: bold; font-size: 0.9rem;">Dezactivează</button>`
                    }
                </div>
            </div>
        `;
...
window.setDeviceStatus = async function (id, newStatus) {
    const { error } = await window.supabaseClient
        .from('active_devices')
...
window.renameDevice = async function (id, currentName) {
    const newName = prompt("Introduceți un nume nou pentru tabletă (ex: Tableta 1):", currentName);
    if (newName && newName.trim() !== "") {
        ...
        if (error) {
            alert("Eroare la redenumire: " + error.message);
        } else {
            window.loadActiveDevices();
        }
    }
...
// Subscriere la realtime pentru tablete (detectăm instant orice tabletă nouă)
if (window.supabaseClient) {
    window.supabaseClient.channel('active_devices_owner')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'active_devices' }, (payload) => {
            ...
            if (payload.eventType === 'INSERT' && payload.new) {
                const devId = payload.new.device_id;
