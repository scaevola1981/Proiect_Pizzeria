import"./supabase-Cnp1ATid.js";import"./security-Bv09CpAF.js";import"./app-wBbXJArf.js";let m=[];async function P(){const e=document.getElementById("login-overlay"),n=document.getElementById("btn-owner-login"),a=document.getElementById("btn-owner-logout"),i=document.getElementById("owner-email"),o=document.getElementById("owner-password"),t=document.getElementById("owner-login-error"),{authenticated:c}=await window.getAuthSession();if(c)e.style.display="none",S();else{const r=document.getElementById("auth-loading"),l=document.getElementById("login-form-content");r&&(r.style.display="none"),l&&(l.style.display="block")}async function s(){const r=i.value.trim(),l=o.value,d=window.checkLoginRateLimit();if(d.blocked){t.style.display="block",t.innerText=`Prea multe încercări. Așteptați ${d.remainingSeconds} secunde.`;return}if(!r||!l){t.style.display="block",t.innerText="Completați email-ul și parola.";return}if(n.disabled=!0,n.innerHTML='<i class="fas fa-spinner fa-spin"></i> Se autentifică...',(await window.loginAdmin(r,l)).success)window.resetLoginAttempts(),t.style.display="none",e.style.display="none",S();else{const p=window.recordFailedLogin();t.style.display="block",p.blocked?t.innerText="Cont blocat temporar. Așteptați 5 minute.":t.innerText=`Email sau parolă incorectă. Mai aveți ${p.attemptsLeft} încercări.`}n.disabled=!1,n.innerHTML='<i class="fas fa-sign-in-alt"></i> Autentificare'}n&&n.addEventListener("click",s),o&&o.addEventListener("keypress",r=>{r.key==="Enter"&&s()}),i&&i.addEventListener("keypress",r=>{r.key==="Enter"&&s()}),a&&a.addEventListener("click",async()=>{await window.supabaseClient.auth.signOut(),window.location.reload()})}async function S(){if(!window.supabaseClient)return;const{authenticated:e}=await window.getAuthSession();if(!e){document.getElementById("login-overlay").style.display="flex";const i=document.getElementById("auth-loading"),o=document.getElementById("login-form-content");i&&(i.style.display="none"),o&&(o.style.display="block");return}const{data:n,error:a}=await window.supabaseClient.from("comenzi").select("*").order("created_at",{ascending:!1});if(a){console.error("Eroare:",a);return}m=n||[],m.forEach(i=>T(i)),renderOwnerOrders(),window.ownerChannelSubscribed||(window.ownerChannelSubscribed=!0,window.supabaseClient.channel("owner_channel").on("postgres_changes",{event:"*",schema:"public",table:"comenzi"},i=>{if(i.eventType==="INSERT")m.findIndex(t=>t.id===i.new.id)===-1&&m.unshift(i.new),T(i.new);else if(i.eventType==="UPDATE"){const o=m.findIndex(t=>t.id===i.new.id);o>-1?m[o]=i.new:m.unshift(i.new),T(i.new)}else i.eventType==="DELETE"&&(m=m.filter(o=>o.id!==i.old.id));renderOwnerOrders()}).subscribe()),window.ownerPollInterval||(window.ownerPollInterval=setInterval(async()=>{if(window.supabaseClient){const{data:i}=await window.supabaseClient.from("comenzi").select("*").order("created_at",{ascending:!1});i&&(m=i,m.forEach(o=>T(o)),renderOwnerOrders())}},5e3))}const O=new Set;function T(e){if(!e||e.status!=="noua")return;const n=Array.isArray(e.detalii_comanda)?e.detalii_comanda.length:0,a=`${e.id}_${e.total}_${n}`;O.has(a)||(O.add(a),console.log("🖨️ Auto-print declanșat automat pentru comanda #",e.id,"Masa:",e.numar_masa),M(e))}window.renderOwnerOrders=function(){const e=document.getElementById("comenzi-container");if(!e)return;e.innerHTML="";const n=new Date;n.setHours(0,0,0,0);let a=0;if(m.length===0){e.innerHTML="<p>Nicio comandă înregistrată.</p>";return}m.forEach(t=>{if(t.status!=="finalizata"&&(a+=parseFloat(t.total)||0),t.status==="finalizata")return;const c=document.createElement("div");c.className="modern-card",t.status,t.status;let s=D(t.detalii_comanda);const r=new Date(t.created_at).toLocaleDateString("ro-RO",{weekday:"short",day:"numeric",month:"short"}),l=new Date(t.created_at).toLocaleTimeString("ro-RO",{hour:"2-digit",minute:"2-digit"});let d=t.status.toUpperCase(),f="#f39c12";t.status==="noua"?(d="NOUĂ",f="#f39c12"):t.status==="in_preparare"?(d="ÎN PREPARARE",f="#2ecc71"):t.status==="servita"&&(d="SERVITĂ",f="#95a5a6");let p="";t.status==="noua"?p=`<button class="modern-card-btn" onclick="window.updateOrderStatus(${parseInt(t.id)}, 'in_preparare')"><i class="fas fa-check"></i> Acceptă Comanda</button>`:t.status==="in_preparare"?p=`<button class="modern-card-btn success" onclick="window.updateOrderStatus(${parseInt(t.id)}, 'servita')"><i class="fas fa-flag-checkered"></i> Marchează ca Servită</button>`:t.status==="servita"&&(p=`<button class="modern-card-btn success" style="background: #e74c3c;" onclick="window.updateOrderStatus(${parseInt(t.id)}, 'finalizata')"><i class="fas fa-broom"></i> Eliberează Masa ${escapeHTML(String(t.numar_masa))}</button>`),c.innerHTML=`
        ...
        `,e.appendChild(c)});const i=m.filter(t=>new Date(t.created_at)>=n&&t.status!=="finalizata");if(i.length>0||a>0){const t=document.createElement("div");t.style.gridColumn="1 / -1",t.innerHTML=`<h2 style="margin-bottom:20px; color:#2ecc71; text-align: center;">Încasări Azi: ${a.toFixed(2)} Lei</h2>`,e.insertBefore(t,e.firstChild)}const o=document.getElementById("btn-incheiere-zi");o&&(o.style.display=i.length>0?"inline-block":"none"),window.renderHistory()};window.renderHistory=function(){const e=document.getElementById("history-content");if(!e)return;const n=new Date;n.setDate(n.getDate()-7);const a=m.filter(t=>new Date(t.created_at)>=n);let i="",o="";if(a.length===0)i='<p style="text-align:center; margin-top:20px;">Nu există comenzi în ultimele 7 zile.</p>';else{const t={};a.forEach(c=>{const s=new Date(c.created_at).toLocaleDateString("ro-RO",{weekday:"long",day:"numeric",month:"long",year:"numeric"});t[s]=(t[s]||0)+(parseFloat(c.total)||0)}),a.forEach(c=>{const s=new Date(c.created_at),r=s.toLocaleDateString("ro-RO",{weekday:"short",day:"numeric",month:"short"}),l=s.toLocaleTimeString("ro-RO",{hour:"2-digit",minute:"2-digit"}),d=s.toLocaleDateString("ro-RO",{weekday:"long",day:"numeric",month:"long",year:"numeric"});if(d!==o){const p=t[d]?t[d].toFixed(2):"0.00";i+=`<div style="grid-column: 1 / -1; border-bottom: 2px solid rgba(255,255,255,0.2); padding-bottom: 10px; margin-top: 20px; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;">
                         ...
                         </div>`,o=d}let f=D(c.detalii_comanda);i+=`
...
Toate comenzile au fost mutate în Istoric.${r}`)}else alert(`Ziua a fost închisă, dar ${o} comenzi au avut erori. Verificați istoricul.`)};window.toggleHistory=e=>{const n=document.getElementById("receptie-panel"),a=document.getElementById("istoric-panel");e?(n.style.display="none",a.style.display="block"):(n.style.display="block",a.style.display="none")};let E;const y=document.getElementById("install-app-btn");function C(){const e=window.matchMedia("(display-mode: standalone)").matches||window.navigator.standalone===!0;y&&(e?y.style.display="none":y.style.display="inline-flex")}C();window.addEventListener("beforeinstallprompt",e=>{e.preventDefault(),E=e,C()});window.addEventListener("appinstalled",()=>{console.log("🎉 PWA Recepție instalată cu succes!"),y&&(y.style.display="none")});y&&y.addEventListener("click",async()=>{if(E){E.prompt();const{outcome:e}=await E.userChoice;console.log(`PWA install choice: ${e}`),e==="accepted"&&(y.style.display="none"),E=null}else alert(`Pentru a instala aplicația pe ecranul principal:
...
• Pe Android / PC (Chrome): Apasă Meniu ⁝ -> Instalează aplicația`)});function D(e){if(!e||!Array.isArray(e)||e.length===0)return'<p style="color: #cbd5e1; font-style: italic;">Fără detalii</p>';const n={};e.forEach(t=>{const c=t.customer_name&&t.customer_name.trim()!==""?t.customer_name:"Masa";n[c]||(n[c]=[]),n[c].push(t)});let a="";const i=["#f5b041","#3498db","#9b59b6","#2ecc71","#e67e22","#1abc9c"];let o=0;for(const[t,c]of Object.entries(n)){let s=0;const r=c.map(p=>{const u=parseFloat(p.product.pret||0),g=parseInt(p.quantity||1),w=u*g;s+=w;const b=p.notes?`<br><small style="color: #e74c3c; font-weight: bold;">* Observații: ${escapeHTML(p.notes)}</small>`:"";return`<div style="color: #fff; font-size: 0.9rem; margin-bottom: 4px; display: flex; justify-content: space-between; align-items: center;">
        ...
        `}return a}async function M(e){if(!e)return;const n=e.detalii_comanda||[],a=String(e.numar_masa||"?"),i=parseFloat(e.total||0).toFixed(2),o=new Date(e.created_at),t=o.toLocaleDateString("ro-RO",{day:"2-digit",month:"2-digit",year:"numeric"}),c=o.toLocaleTimeString("ro-RO",{hour:"2-digit",minute:"2-digit"}),s=n.some(u=>u.is_new===!0),r=s,l=s?n.filter(u=>u.is_new===!0):n,d={};l.forEach(u=>{const g=u.customer_name&&u.customer_name.trim()!==""?u.customer_name:"Masa";d[g]||(d[g]=[]),d[g].push(u)});let f="";for(const[u,g]of Object.entries(d)){const w=u==="Masa"?"👥 Împreună":`👤 ${u}`;let b=0;f+=`<div style="border-top: 1px dashed #000; padding: 4px 0 2px; margin-top: 4px;">
            <b>${w}</b>
        </div>`,g.forEach(x=>{var $,I;const z=parseInt(x.quantity||1),A=parseFloat((($=x.product)==null?void 0:$.pret)||0),L=z*A;b+=L;const B=x.notes?`<br><small><i>* ${x.notes}</i></small>`:"";f+=`<div style="display: flex; justify-content: space-between; font-size: 12px; padding: 1px 0;">
                <span>${z}x ${((I=x.product)==null?void 0:I.nume)||"Produs"}</span>
    ...
    <body>
        ...
        <div class="footer">
    ...
    </html>`;if(typeof qz<"u"&&qz.websocket.isActive())try{const u=h||await qz.printers.getDefault(),g=qz.print.createConfig(u),w=[{type:"pixel",format:"html",flavor:"plain",data:p}];qz.print(g,w).then(()=>{console.log("✅ Bon printat silențios via QZ Tray pe:",u)}).catch(b=>{console.warn("Eroare printare QZ Tray, fallback la browser:",b),k(p)});return}catch(u){console.warn("Excepție QZ Tray, fallback la browser:",u)}k(p)}function k(e){let n=document.getElementById("receipt-print-frame");n||(n=document.createElement("iframe"),n.id="receipt-print-frame",n.style.cssText="position: fixed; top: -10000px; left: -10000px; width: 80mm; height: 0; border: none; visibility: hidden;",document.body.appendChild(n));const a=n.contentDocument||n.contentWindow.document;a.open(),a.write(e),a.close(),n.onload=()=>{try{n.contentWindow.focus(),n.contentWindow.print()}catch(i){console.warn("Printare automată blocată, se deschide fereastra manuală:",i);const o=window.open("","_blank","width=320,height=600");o&&(o.document.write(e),o.document.close(),o.onload=()=>{o.print(),o.onafterprint=()=>o.close()})}}}let h=null;async function _(){if(typeof qz>"u")return v("Lipsă SDK QZ","#e74c3c"),!1;if(qz.websocket.isActive())return v(`Conectat (${h||"Implicită"})`,"#2ecc71"),!0;try{qz.security.setCertificatePromise(e=>e()),qz.security.setSignaturePromise(()=>e=>e()),await qz.websocket.connect({retries:3,delay:1});try{const e=await qz.printers.find();console.log("🖨️ Imprimante detectate de QZ Tray:",e);const n=e.find(a=>a.toLowerCase().includes("samsung")||a.toLowerCase().includes("m2020")||a.toLowerCase().includes("m2026"));n?h=n:h=await qz.printers.getDefault(),v(`Conectat: ${h||"Implicită"}`,"#2ecc71")}catch{v("Conectat QZ","#2ecc71")}return!0}catch(e){return console.warn("QZ Tray nu este pornit pe laptop:",e),v("Deconectat (Offline)","#f5b041"),!1}}function v(e,n){const a=document.getElementById("qz-status-badge");a&&(a.style.background=`rgba(${n==="#2ecc71"?"46, 204, 113":"245, 176, 65"}, 0.2)`,a.style.color=n,a.style.borderColor=n,a.innerHTML=`<i class="fas fa-print"></i> QZ Tray: ${e}`)}window.testQZPrint=async function(){if(await _()&&typeof qz<"u"&&qz.websocket.isActive())try{const n=h||await qz.printers.getDefault(),a=qz.print.createConfig(n),o=[{type:"pixel",format:"html",flavor:"plain",data:`
            ...
            `}];await qz.print(a,o),alert(`✅ Test trimis cu succes pe imprimanta: ${n}!`)}catch(n){alert("Eroare la printare prin QZ Tray: "+n.message)}else alert("QZ Tray nu este pornit pe laptop. Deschide aplicația QZ Tray pe Windows!")};window.printOrderReceipt=function(e){const n=m.find(a=>a.id===e);n&&(M(n),n.status==="noua"&&window.updateOrderStatus(e,"in_preparare"))};P();setTimeout(_,1e3);
