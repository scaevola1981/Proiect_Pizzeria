...
• Pe Android / PC (Chrome): Apasă Meniu ⁝ -> Instalează aplicația`)});function R(e){if(!e||!Array.isArray(e)||e.length===0)return'<p style="color: #cbd5e1; font-style: italic;">Fără detalii</p>';const i={};e.forEach(t=>{const c=t.customer_name&&t.customer_name.trim()!==""?t.customer_name:"Masa";i[c]||(i[c]=[]),i[c].push(t)});let n="";const a=["#f5b041","#3498db","#9b59b6","#2ecc71","#e67e22","#1abc9c"];let s=0;for(const[t,c]of Object.entries(i)){let o=0;const r=c.map(u=>{const x=parseFloat(u.product.pret||0),b=parseInt(u.quantity||1),y=x*b;o+=y;const l=u.notes?`<br><small style="color: #e74c3c; font-weight: bold;">* Observații: ${escapeHTML(u.notes)}</small>`:"";return`<div style="color: #fff; font-size: 0.9rem; margin-bottom: 4px; display: flex; justify-content: space-between; align-items: center;">
    ...
    </html>`;if(await B()&&typeof qz<"u"&&qz.websocket.isActive())try{const l=v||await qz.printers.getDefault(),g=qz.configs.create(l),L=[{type:"pixel",format:"html",flavor:"plain",data:x}];await qz.print(g,L),console.log("✅ Bon printat 100% silențios via QZ Tray pe:",l);return}catch(l){console.warn("❌ Eroare trimitere job QZ Tray:",l)}else console.warn("⚠️ QZ Tray nu este conectat — bonul NU a fost printat. Pornește QZ Tray pe laptop!")}let v=null,P=!1,T=!1;function j(){if(!(P||typeof qz>"u")){P=!0;var e=`-----BEGIN CERTIFICATE-----
...
-----END RSA PRIVATE KEY-----`;qz.security.setCertificatePromise(function(n,a){n(e)}),qz.security.setSignaturePromise(function(n){return function(a,s){try{var t=KEYUTIL.getKey(i),c=new KJUR.crypto.Signature({alg:"SHA512withRSA"});c.init(t),c.updateString(n);var o=c.sign();a(stob64(hextorstr(o)))}catch(r){console.error("QZ Tray semnare eșuată:",r),a()}}}),qz.websocket.setClosedCallbacks(function(n){console.warn("⚠️ QZ Tray: Conexiune pierdută. Se reconectează în 3 secunde...",n),h("Reconectare...","#f5b041"),setTimeout(S,3e3)}),console.log("✅ QZ Tray: Securitate configurată cu certificat digital")}}async function S(){if(typeof qz>"u")return!1;if(qz.websocket.isActive())return await H(),h(`Conectat (${v||"Implicită"})`,"#2ecc71"),!0;if(T)return!1;T=!0;try{return j(),await qz.websocket.connect({retries:3,delay:1}),console.log("✅ QZ Tray: WebSocket conectat cu succes!"),await H(),h(`Conectat (${v||"Implicită"})`,"#2ecc71"),T=!1,!0}catch(e){return console.warn("❌ QZ Tray: Nu s-a putut conecta (aplicația QZ Tray pornită pe laptop?):",e.message||e),h("Deconectat","#f5b041"),T=!1,!1}}async function H(){if(qz.websocket.isActive())try{const e=await qz.printers.find();console.log("🖨️ QZ Tray: Imprimante detectate:",e),v=e.find(n=>n&&(n.toLowerCase().includes("ocpp")||n.toLowerCase().includes("pos-80")||n.toLowerCase().includes("pos 80")||n.toLowerCase().includes("pos80")||n.toLowerCase().includes("thermal")||n.toLowerCase().includes("receipt")||n.toLowerCase().includes("samsung")||n.toLowerCase().includes("m2020")||n.toLowerCase().includes("m2026")||n.toLowerCase().includes("xprinter")||n.toLowerCase().includes("epson")))||await qz.printers.getDefault(),console.log("🖨️ QZ Tray: Imprimantă selectată:",v)}catch(e){console.warn("QZ Tray: Nu s-au putut detecta imprimantele:",e)}}let $=!1;async function z(){try{const e=await fetch("http://localhost:4000/status",{signal:AbortSignal.timeout(1500)});if(e.ok){const i=await e.json();return $=!0,h(`In-House USB Activ (${i.printer||"POS-80"})`,"#2ecc71"),!0}}catch{$=!1}return!1}async function B(){return await z()?!0:typeof qz>"u"?(h("Deconectat","#e74c3c"),!1):await S()}function h(e,i){const n=document.getElementById("qz-status-badge");if(n){const a=i==="#2ecc71"?"46, 204, 113":i==="#e74c3c"?"231, 76, 60":"245, 176, 65";n.style.background=`rgba(${a}, 0.2)`,n.style.color=i,n.style.borderColor=i,n.innerHTML=`<i class="fas fa-print"></i> Print: ${e}`}}window.testQZPrint=async function(){if(await z())try{const a=await(await fetch("http://localhost:4000/test-print")).json();if(a.success){alert(`✅ Test trimis cu succes prin Serviciul In-House pe imprimanta USB "${a.printer||"POS-80"}"!
...
2. Apăsați din nou Test`)};window.printOrderReceipt=function(e){const i=m.find(n=>n.id===e);i&&(Z(i),i.status==="noua"&&window.updateOrderStatus(e,"in_preparare"))};U();setTimeout(B,1e3);setInterval(async()=>{!await z()&&typeof qz<"u"&&!qz.websocket.isActive()&&!T&&(console.log("🔄 Keep-alive QZ Tray reconectare..."),await S())},8e3);
