...
• Pe Android / PC (Chrome): Apasă Meniu ⁝ -> Instalează aplicația`)});function P(t){if(!t||!Array.isArray(t)||t.length===0)return'<p style="color: #cbd5e1; font-style: italic;">Fără detalii</p>';const a={};t.forEach(e=>{const c=e.customer_name&&e.customer_name.trim()!==""?e.customer_name:"Masa";a[c]||(a[c]=[]),a[c].push(e)});let n="";const i=["#f5b041","#3498db","#9b59b6","#2ecc71","#e67e22","#1abc9c"];let o=0;for(const[e,c]of Object.entries(a)){let s=0;const r=c.map(m=>{const T=parseFloat(m.product.pret||0),p=parseInt(m.quantity||1),g=T*p;s+=g;const b=m.notes?`<br><small style="color: #e74c3c; font-weight: bold;">* Observații: ${escapeHTML(m.notes)}</small>`:"";return`<div style="color: #fff; font-size: 0.9rem; margin-bottom: 4px; display: flex; justify-content: space-between; align-items: center;">
        ...
        `}return n}async function q(t){if(!t)return;const a=t.detalii_comanda||[],n=String(t.numar_masa||"?"),i=parseFloat(t.total||0).toFixed(2),o=new Date(t.created_at),e=o.toLocaleDateString("ro-RO",{day:"2-digit",month:"2-digit",year:"numeric"}),c=o.toLocaleTimeString("ro-RO",{hour:"2-digit",minute:"2-digit"}),s=a.some(p=>p.is_new===!0),r=s,l=s?a.filter(p=>p.is_new===!0):a,d={};l.forEach(p=>{const g=p.customer_name&&p.customer_name.trim()!==""?p.customer_name:"Masa";d[g]||(d[g]=[]),d[g].push(p)});let f="";for(const[p,g]of Object.entries(d)){const b=p==="Masa"?"👥 Împreună":`👤 ${p}`;let z=0;f+=`<div style="border-top: 1px dashed #000; padding: 4px 0 2px; margin-top: 4px;">
            <b>${b}</b>
        </div>`,g.forEach(h=>{var S,M;const B=parseInt(h.quantity||1),R=parseFloat(((S=h.product)==null?void 0:S.pret)||0),C=B*R;z+=C;const H=((M=h.product)==null?void 0:M.nume)||"Produs",Z=h.notes?`<div style="font-size: 11px; font-style: italic; padding-left: 8px;">* ${h.notes}</div>`:"";f+=`<div style="display: flex; justify-content: space-between; font-size: 13px; font-weight: bold; padding: 2px 0; line-height: 1.3;">
                <span style="font-weight: bold;"><b>${B}x ${H}</b></span>
                <span style="font-weight: bold;">${C.toFixed(2)}</span>
            </div>${Z}`}),f+=`<div style="text-align: right; font-size: 11px; font-weight: bold; padding-top: 2px;">Subtotal: ${z.toFixed(2)} Lei</div>`}const m=`
    <!DOCTYPE html>
    <html>
    <head>
        <meta charset="UTF-8">
...
3. Apasă din nou Test`)};window.printOrderReceipt=function(t){const a=u.find(n=>n.id===t);a&&(q(a),a.status==="noua"&&window.updateOrderStatus(t,"in_preparare"))};$();setTimeout(L,1e3);setInterval(async()=>{typeof qz<"u"&&!qz.websocket.isActive()&&!x&&(console.log("🔄 QZ Tray: Keep-alive reconectare..."),await E())},1e4);
