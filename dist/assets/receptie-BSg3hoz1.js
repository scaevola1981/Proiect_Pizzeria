...
• Pe Android / PC (Chrome): Apasă Meniu ⁝ -> Instalează aplicația`)});function P(t){if(!t||!Array.isArray(t)||t.length===0)return'<p style="color: #cbd5e1; font-style: italic;">Fără detalii</p>';const n={};t.forEach(e=>{const c=e.customer_name&&e.customer_name.trim()!==""?e.customer_name:"Masa";n[c]||(n[c]=[]),n[c].push(e)});let a="";const i=["#f5b041","#3498db","#9b59b6","#2ecc71","#e67e22","#1abc9c"];let o=0;for(const[e,c]of Object.entries(n)){let s=0;const r=c.map(m=>{const z=parseFloat(m.product.pret||0),p=parseInt(m.quantity||1),g=z*p;s+=g;const b=m.notes?`<br><small style="color: #e74c3c; font-weight: bold;">* Observații: ${escapeHTML(m.notes)}</small>`:"";return`<div style="color: #fff; font-size: 0.9rem; margin-bottom: 4px; display: flex; justify-content: space-between; align-items: center;">
        ...
        `}return a}async function q(t){if(!t)return;const n=t.detalii_comanda||[],a=String(t.numar_masa||"?"),i=parseFloat(t.total||0).toFixed(2),o=new Date(t.created_at),e=o.toLocaleDateString("ro-RO",{day:"2-digit",month:"2-digit",year:"numeric"}),c=o.toLocaleTimeString("ro-RO",{hour:"2-digit",minute:"2-digit"}),s=n.some(p=>p.is_new===!0),r=s,l=s?n.filter(p=>p.is_new===!0):n,d={};l.forEach(p=>{const g=p.customer_name&&p.customer_name.trim()!==""?p.customer_name:"Masa";d[g]||(d[g]=[]),d[g].push(p)});let f="";for(const[p,g]of Object.entries(d)){const b=p==="Masa"?"👥 Împreună":`👤 ${p}`;let L=0;f+=`<div style="border-top: 1px dashed #000; padding: 4px 0 2px; margin-top: 4px;">
            <b>${b}</b>
        </div>`,g.forEach(h=>{var M,C;const B=parseInt(h.quantity||1),R=parseFloat(((M=h.product)==null?void 0:M.pret)||0),S=B*R;L+=S;const H=h.notes?`<br><small><i>* ${h.notes}</i></small>`:"";f+=`<div style="display: flex; justify-content: space-between; font-size: 12px; padding: 1px 0;">
                <span>${B}x ${((C=h.product)==null?void 0:C.nume)||"Produs"}</span>
    ...
    </html>`;if(await E()&&typeof qz<"u"&&qz.websocket.isActive())try{const p=w||await qz.printers.getDefault(),g=qz.configs.create(p),b=[{type:"pixel",format:"html",flavor:"plain",data:m}];await qz.print(g,b),console.log("✅ Bon printat 100% silențios via QZ Tray pe:",p);return}catch(p){console.warn("❌ Eroare trimitere job QZ Tray:",p)}else console.warn("⚠️ QZ Tray nu este conectat — bonul NU a fost printat. Pornește QZ Tray pe laptop!")}let w=null,N=!1,x=!1;function $(){if(!(N||typeof qz>"u")){N=!0;var t=`-----BEGIN CERTIFICATE-----
...
-----END RSA PRIVATE KEY-----`;qz.security.setCertificatePromise(function(a,i){a(t)}),qz.security.setSignaturePromise(function(a){return function(i,o){try{var e=KEYUTIL.getKey(n),c=new KJUR.crypto.Signature({alg:"SHA512withRSA"});c.init(e),c.updateString(a);var s=c.sign();i(stob64(hextorstr(s)))}catch(r){console.error("QZ Tray semnare eșuată:",r),i()}}}),qz.websocket.setClosedCallbacks(function(a){console.warn("⚠️ QZ Tray: Conexiune pierdută. Se reconectează în 3 secunde...",a),I("Reconectare...","#f5b041"),setTimeout(T,3e3)}),console.log("✅ QZ Tray: Securitate configurată cu certificat digital")}}async function T(){if(typeof qz>"u")return!1;if(qz.websocket.isActive())return await O(),I(`Conectat (${w||"Implicită"})`,"#2ecc71"),!0;if(x)return!1;x=!0;try{return $(),await qz.websocket.connect({retries:3,delay:1}),console.log("✅ QZ Tray: WebSocket conectat cu succes!"),await O(),I(`Conectat (${w||"Implicită"})`,"#2ecc71"),x=!1,!0}catch(t){return console.warn("❌ QZ Tray: Nu s-a putut conecta (aplicația QZ Tray pornită pe laptop?):",t.message||t),I("Deconectat","#f5b041"),x=!1,!1}}async function O(){if(qz.websocket.isActive())try{const t=await qz.printers.find();console.log("🖨️ QZ Tray: Imprimante detectate:",t),w=t.find(a=>a&&(a.toLowerCase().includes("samsung")||a.toLowerCase().includes("m2020")||a.toLowerCase().includes("m2026")))||await qz.printers.getDefault(),console.log("🖨️ QZ Tray: Imprimantă selectată:",w)}catch(t){console.warn("QZ Tray: Nu s-au putut detecta imprimantele:",t)}}async function E(){return typeof qz>"u"?(I("Lipsă SDK QZ","#e74c3c"),!1):await T()}function I(t,n){const a=document.getElementById("qz-status-badge");if(a){const i=n==="#2ecc71"?"46, 204, 113":n==="#e74c3c"?"231, 76, 60":"245, 176, 65";a.style.background=`rgba(${i}, 0.2)`,a.style.color=n,a.style.borderColor=n,a.innerHTML=`<i class="fas fa-print"></i> QZ Tray: ${t}`}}window.testQZPrint=async function(){if(await E()&&qz.websocket.isActive())try{const n=w||await qz.printers.getDefault(),a=qz.configs.create(n),o=[{type:"pixel",format:"html",flavor:"plain",data:`
...
3. Apasă din nou Test`)};window.printOrderReceipt=function(t){const n=u.find(a=>a.id===t);n&&(q(n),n.status==="noua"&&window.updateOrderStatus(t,"in_preparare"))};Z();setTimeout(E,1e3);setInterval(async()=>{typeof qz<"u"&&!qz.websocket.isActive()&&!x&&(console.log("🔄 QZ Tray: Keep-alive reconectare..."),await T())},1e4);
