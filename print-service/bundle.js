...
// escpos-builder.js
var require_escpos_builder = __commonJS({
  "escpos-builder.js"(exports2, module2) {
    ...
    function buildEscPosBuffer2(order, options = {}) {
      ...
      commands.push(Buffer.from([27, 116, 18]));
...
`, "ascii"));
      ...
      if (beepOnOrder) {
        commands.push(Buffer.from([27, 66, Math.min(9, Math.max(1, beepCount)), 2]));
      }
...
var require_printer_driver_usb = __commonJS({
  "printer-driver-usb.js"(exports2, module2) {
    ...
    async function resolveTargetPrinter2(config2 = {}) {
      if (cachedPrinterName) return cachedPrinterName;
      if (process.platform !== "win32") {
        cachedPrinterName = config2.printer_name || "POS-80";
  ...
  try {
    ...
    if (order.status === "noua") {
      try {
        ...
        console.log(`\u{1F504} Statusul comenzii #${order.id} actualizat \xEEn "in_preparare".`);
      } catch (dbErr) {
...
async function syncUnprintedOrders() {
  console.log("\u{1F50D} Verificare comenzi neprintate \xEEn Supabase (Startup Recovery)...");
  try {
    const { data: orders, error } = await supabase.from("comenzi").select("*").eq("status", "noua").order("created_at", { ascending: true });
    if (error) {
      console.error("\u274C Eroare la c\u0103utarea comenzilor neprintate:", error.message);
      return;
    }
    if (orders && orders.length > 0) {
      console.log(`\u{1F4CB} S-au g\u0103sit ${orders.length} comenzi neprintate. Se trimit spre imprimant\u0103...`);
...
function initRealtimeSubscription() {
  ...
  supabase.channel("bella-print-channel").on("postgres_changes", { event: "INSERT", schema: "public", table: "comenzi" }, (payload) => {
    console.log("\u{1F514} [REALTIME INSERT] Comand\u0103 nou\u0103 primit\u0103:", payload.new.id);
    ...
    if (payload.new && payload.new.status === "noua") {
      console.log("\u{1F514} [REALTIME UPDATE] Comand\u0103 suplimentat\u0103 primit\u0103:", payload.new.id);
      processOrder(payload.new);
    }
  }).subscribe((status) => {
    console.log(`\u{1F4E1} Status canal Realtime: ${status}`);
  });
}
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === "/test-print") {
      const testOrder = {
        ...
        total: 55,
        detalii_comanda: [
          { product: { nume: "Pizza Diavola", pret: 35 }, quantity: 1, notes: "Test Print Reusit", customer_name: "Masa" },
          { product: { nume: "Coca-Cola 0.33L", pret: 10 }, quantity: 2, customer_name: "Masa" }
        ]
      };
...
`);
  ...
  if (config.poll_interval_seconds > 0) {
    setInterval(syncUnprintedOrders, config.poll_interval_seconds * 1e3);
  }
  console.log("\u2728 Serviciul ruleaz\u0103 \xEEn fundal \u0219i ascult\u0103 comenzi...");
}
